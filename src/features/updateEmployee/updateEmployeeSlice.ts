...
function loadingFailed(state: employeeToPutState, action: PayloadAction<string>) {
    ...
    reducers: {
        ...
        setEmployeeToPut(state, action: PayloadAction<employeeToPut>) {
            const {id,first_name, last_name, phone, 
                   email, address, city, st, zip, notes} = action.payload
            state.id = id
...
export const putEmployee = (id: number, first_name: string, last_name: string, phone: string, 
                              email: string, address: string, city: string, 
                              st: string, zip: string, notes: string): AppThunk => async dispatch => {
    const payload = {
